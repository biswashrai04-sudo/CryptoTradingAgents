...
def get_investment_preferences():
    ...
    try:
        ...
        logger.warning(
            "Investment preferences file not found. Using default preferences."
        )
        return ""
...
def run_analysis(
    ...
    logger.add(
        "logs/reports.log", rotation="1 day", retention="7 days", level="SUCCESS"
    )
    graph = TradingAgentsGraph(
        [analyst.value for analyst in analysts], config=DEFAULT_CONFIG
    )
    ...
    for chunk in graph.graph.stream(init_agent_state, **args):
        if len(chunk["messages"]) > 0:
            ...
            if (
                "market_report" in chunk
                and chunk["market_report"]
                and completion_status.get("market_report", False) is False
            ):
            ...
            if (
                "sentiment_report" in chunk
                and chunk["sentiment_report"]
                and completion_status.get("sentiment_report", False) is False
            ):
            ...
            if (
                "news_report" in chunk
                and chunk["news_report"]
                and completion_status.get("news_report", False) is False
            ):
            ...
            if (
                "fundamentals_report" in chunk
                and chunk["fundamentals_report"]
                and completion_status.get("fundamentals_report", False) is False
            ):
                logger.info("--- Fundamentals Analysis Completed ---")
            ...
            if (
                ...
                and completion_status.get("investment_debate_state", False) is False
            ):
                debate_state = chunk["investment_debate_state"]
                if (
                    "bull_history" in debate_state
                    and debate_state["bull_history"]
                    and completion_status.get("bull_history", False) is False
                ):
                ...
                if (
                    "bear_history" in debate_state
                    and debate_state["bear_history"]
                    and completion_status.get("bear_history", False) is False
                ):
                    logger.info("--- Research Team Debate (In Progress) ---")
                ...
                if debate_state.get("judge_decision"):
                    logger.info("--- Research Team Debate Completed ---")
                    logger.success(
                        f"Investment Judge Decision: {debate_state['judge_decision']}"
                    )
            ...
            if (
                ...
                and completion_status.get("trader_investment_plan", False) is False
            ):
                logger.info("--- Trader Investment Planning Completed ---")
                logger.success(
                    f"Trader Investment Plan: {chunk['trader_investment_plan']}"
                )
                completion_status["trader_investment_plan"] = True

            if (
                "risk_debate_state" in chunk
                and chunk["risk_debate_state"]
                and completion_status.get("risk_debate_state", False) is False
            ):
                risk_state = chunk["risk_debate_state"]
                if (
                    "current_risky_response" in risk_state
                    and risk_state["current_risky_response"]
                    and completion_status.get("current_risky_response", False) is False
                ):
                    logger.info("--- Risk Discussion - Risky (In Progress) ---")
                    logger.success(
                        f"Current Risky Response: {risk_state['current_risky_response']}"
                    )
                    completion_status["current_risky_response"] = True
                if (
                    "current_safe_response" in risk_state
                    and risk_state["current_safe_response"]
                    and completion_status.get("current_safe_response", False) is False
                ):
                    logger.info("--- Risk Discussion - Safe (In Progress) ---")
                    logger.success(
                        f"Current Safe Response: {risk_state['current_safe_response']}"
                    )
                    completion_status["current_safe_response"] = True
                if (
                    "current_neutral_response" in risk_state
                    and risk_state["current_neutral_response"]
                    and completion_status.get("current_neutral_response", False)
                    is False
                ):
                    logger.info("--- Risk Discussion - Neutral (In Progress) ---")
                    logger.success(
                        f"Current Neutral Response: {risk_state['current_neutral_response']}"
                    )
                    completion_status["current_neutral_response"] = True

                if (
                    "judge_decision" in risk_state
                    and risk_state["judge_decision"]
                    and completion_status.get("risk_judge_decision", False) is False
                ):
                    logger.info("--- Risk Discussion Completed ---")
                    logger.success(
                        f"Risk Judge Decision: {risk_state['judge_decision']}"
                    )
    ...
    decision_color = (
        "green" if decision == "Buy" else "red" if decision == "Sell" else "yellow"
    )
    ...
    if DEFAULT_CONFIG["save_report"]:
        ...
        save_reports(
            ticker,
            reports,
            DEFAULT_CONFIG["report_dir"],
            DEFAULT_CONFIG["report_type"],
            decision=decision,
        )
